#   ---numerisch hydraulische 1D Abflussmodellierung - PQ-Beziehung, Oktober 2019, kwy---  #
# Abflussmodellierung mit der Fliessformel nach Strickler (Q = v * A)
...
########################################################################
pn = 'Querprofil_Pegel Hasliaare'        # Profilname definieren
fn = 'qpPegel_Hasliaare2.txt'            # file name (fn) definieren. = Name des files in dem das Profil gespeichert ist
h = 70   # [cm]                          # Wasserhoehe definieren, meist abgelesen von eiener Pegelstation
kst = 20                                 # Rauhigkeitsbeiwert nach Strickler definieren (u.a. kalibriert mit Q-Messung)
J = float(0.010785)                      # Fliesssgefaelle definieren (aufgenommen mit DGPS oder aus Drohnen-DEM)
...
for ele in H:
    ...
    profil_hoehe = profil_hoehe_ganz - min(
        profil_hoehe_ganz)  # Hoehen vom kleinsten Hoehenwert subtrahieren, nullsetzen
    ...
    # Im folgenden while-loop sind 15 Bedingungen beschrieben, welche bei der Schnittpunktberechnung eintreffen koennen.
    # z 44-46, 66, 71, 73-84 ausschalten um NUR die Punkte zu beruechsichtigen, welche effektiv berechnet werden muessen / jedoch sind alle Pkt. noetig im benetzten Umfang bzw. im Loop danach!
    ...
    while i <= ((len(x_h_tuple)) - 1):  # Wenn der index (i) kleiner oder gleich der Tuple-Listenlaenge ist mach:
        ...
        # Pkt. (i) im Wasser, i-1 auch im Wasser und nun i+1 NICHT im Wasser:
            ...
            # xa = gesucht, hier = x1 (neuer Punkt)
            # x1 = (x_h_tuple[i].item(0))
            # x2 = (x_h_tuple[i+1].item(0))
            # ya = h
            # y1 = (x_h_tuple[i].item(1))
            # y2 = (x_h_tuple[i+1].item(1))
        ...
        # Letzer Punkt in der Liste:
            ...
            profile_list.append(x_h_tuple[i])  # Das ist dann die letzte Position in der Liste (auch diese weg, wenn nicht beruechsichtigt werden soll)
        i = i + 1
    ...
    ########################################################################
    # print("Errechnete Werte bei einem Pegel von " + str(h) + " m:")
    ...
    # print("Die Querschnittsflaeche (A) betraegt: " + str(A_round) + " m^2")
    A_list.append(A)
    ...
    P_round = round(P, 3)
    sum(removelist)
    # print("Der benetzte Umfang (P) betraegt: " + str(P_round) + " m")
    P_list.append(P)
    ...
    # print("Der hydraulische Radius (Rhy) betraegt: " + str(Rhy_round) + " m")
    Rhy_list.append(Rhy)
    ...
    Q_round = round(Q, 3)
    # print("Der Abfluss (Q) betraegt: " + str(Q_round) + " m^3/s")
    Q_H.append(Q)
    # print("Schlaufenende")
...
plt.title('Original und verduenntes QP, bei Pegel ' + str(h_round) + ' m')
plt.legend(loc='best')
...
x2 = [0.575, 0.585, 0.599, 0.608, 0.615, 0.620, 0.645]   # Manuell gemessenen Hoehen
y2 = [3.171, 3.258, 3.755, 4.133, 4.609, 4.609, 5.099]   # Manuell gemessener Abfluss
x3 = np.array(x2)
y3 = np.array(y2)
...
plt.plot(x3, y3, marker='s', color='r', label='gemessen')
plt.legend()
